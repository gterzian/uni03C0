...
///
/// The body closure runs off the main actor: `ProcessController` is an
...
public actor ProcessController {
    ...
    public func terminate() async {
        ...
        await failAllPending(with: ProcessError.userTerminated)
    ...
    private func awaitWriter() async throws -> StandardInputWriter {
        ...
        if didExit { throw ProcessError.disconnected }
    ...
    private func serve(_ execution: Execution<CustomWriteInput, SequenceOutput, SequenceOutput>) async {
        ...
        stderrTask.cancel()
        await failAllPending(with: ProcessError.disconnected)
        didExit = true
        eventsContinuation.finish()
    }

    private func handleFatal(_ error: any Error) async {
        await failAllPending(with: ProcessError.launchFailed(error.localizedDescription))
        eventsContinuation.finish()
    }
